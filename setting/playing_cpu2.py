...
def Playing_cpu2(*ball, menu, play, screen_ratio):
    ...
    while play:
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                play = False
                menu = True
            if event.type == pygame.MOUSEBUTTONDOWN and player_turn == 0:
                click = pygame.mouse.get_pos()
                distance_0 = np.hypot((click[0] - ball[0].pos[0]), (click[1] - ball[0].pos[1]))
                distance_1 = np.hypot((click[0] - spin_ball[0]), (click[1] - spin_ball[1]))
                if distance_0 < ball[0].r:
                    buttonup = (0, 0)
            if event.type == pygame.MOUSEBUTTONUP and player_turn == 0:
                if distance_0 < ball[0].r:
        ...
        if np.all([ball[0].v, ball[1].v, ball[2].v] == np.array([0, 0])):
            ...
            if player_turn == 1:
                ...
                buttonup = (ball[1].pos[0] + distance_2 * np.cos(theta), ball[1].pos[1] + distance_2 * np.sin(theta))
